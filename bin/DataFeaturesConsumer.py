...
def Feature_PreviousReading(file_name, consumer, nb_preceding_time):
    ...
    Target = pd.read_pickle(file_name + '.pkl').iloc[:, consumer]
...
def Feature_MeanConsumption(file_name, consumer):
...
def Feature_MeanPrevious(file_name, consumer, nb_previous_days, 
                         nb_preceding_time):
...
def Feature_LastValues(file_name, consumer, nb_last_values):
...
def main(file_name, weather_file_name):
    ''' This function creates the normalized features for the given cluster and 
    data set '''
    
    Target_df = pd.read_pickle(file_name + '.pkl')
    Weather(weather_file_name)
    Weather_df = pd.read_pickle(weather_file_name + '.pkl')
    Feature_Time_df = Feature_Time(file_name)
    DataFull_df = pd.DataFrame()
    for consumer in range(Target_df.shape[1]):
        Target_one = pd.DataFrame(Target_df.iloc[:, consumer])
        Feature_PreviousReading_df = Feature_PreviousReading(
                file_name, consumer, nb_preceding_time)
        Feature_MeanConsumption_df = Feature_MeanConsumption(
                file_name, consumer)
        Feature_MeanPrevious_df = Feature_MeanPrevious(
                file_name, consumer, nb_previous_days, nb_preceding_time)
        Feature_LastValues_df = Feature_LastValues(
                file_name, consumer, nb_last_values)
        FeaturesFull_df = pd.concat([
                Weather_df, Feature_PreviousReading_df,
                Feature_MeanConsumption_df, Feature_MeanPrevious_df,
                Feature_LastValues_df], axis=1)
        # Normalize the features #
        scaler = StandardScaler()
        FeaturesFull_Normalized_df = pd.DataFrame(scaler.fit_transform(
                FeaturesFull_df.values))
        # Add the time features, the target and the index #
        Index = pd.DataFrame([consumer]*Target_one.shape[0])
        FeaturesFull_Complete_df = pd.concat([
                FeaturesFull_Normalized_df, Feature_Time_df, Target_one,
                Index], axis=1)
        ...
        FeaturesFull_Complete_df = FeaturesFull_Complete_df.reset_index(
                drop=True)
        FeaturesFull_Complete_df.columns = [i for i in range(
                FeaturesFull_Complete_df.shape[1])]
        DataFull_df = pd.concat([DataFull_df, FeaturesFull_Complete_df])
    
    DataFull_df.to_pickle('Features_' + file_name + '.pkl')
...
if __name__ == '__main__':
    ...
    nb_previous_days = 3
    nb_last_values = 1
    main(file_name, weather_file_name)    
